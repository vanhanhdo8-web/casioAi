...
bytes_between_cmds = []  # list of (pos, offset1, label1, offset2, label2)
endaddr = ""
end = []
addr = 1
...
def process(line):
    ...
    global result, labels, adr_of_cmds, adr_arith_cmds, pr_length_cmds, bytes_between_cmds, home, string_vars, in_comment, endaddr, end, count
    line.replace('\n', "")
    count = count + 1
...
"""
    ...
    elif line.startswith('bytes_between'):
        ''' Syntax: `bytes_between [offset1] label1 [offset2] label2`
            Tính số byte từ label1+offset1 đến label2+offset2 (theo little‑endian, 2 byte)
        '''
        parts = line[13:].strip().split()
        if len(parts) not in (2, 4):
            raise ValueError('Sai cú pháp bytes_between. Dùng: bytes_between [offset1] label1 [offset2] label2')

        def parse_part(part):
            part = part.strip()
            # Kiểm tra offset dạng [số]
            if part.startswith('['):
                # Tìm dấu ']'
                end_bracket = part.find(']')
                if end_bracket == -1:
                    raise ValueError(f'Thiếu dấu "]" trong offset: {part}')
                offset_str = part[1:end_bracket].strip()
                try:
                    offset = int(offset_str, 0)
                except ValueError:
                    raise ValueError(f'Offset không hợp lệ: {offset_str}')
                label = part[end_bracket+1:].strip()
                if not label:
                    raise ValueError(f'Thiếu label sau offset: {part}')
                return offset, label
            else:
                # Không có offset
                return 0, part
        ...
        if len(parts) == 2:
            # Không offset
            offset1, label1 = 0, parts[0]
            offset2, label2 = 0, parts[1]
        else:
            # Có offset, parts[0] có thể là "[số]" và parts[1] là label, hoặc gộp lại?
            # Để đơn giản, ta ghép parts[0] và parts[1] để xử lý offset dạng "[số]label"
            # và parts[2] + parts[3] tương tự.
            part1 = parts[0] + ' ' + parts[1] if parts[1] else parts[0]
            part2 = parts[2] + ' ' + parts[3] if len(parts) > 3 and parts[3] else parts[2]
            offset1, label1 = parse_part(part1)
            offset2, label2 = parse_part(part2)

        bytes_between_cmds.append((len(result), offset1, label1, offset2, label2))
        result.extend((0, 0))  # 2 byte placeholder
...
def finish_processing():
    global result, labels, adr_arith_cmds, pr_length_cmds, bytes_between_cmds, addr
    ...
    # Process deferred bytes_between commands
    for pos, offset1, label1, offset2, label2 in bytes_between_cmds:
        if label1 not in labels or label2 not in labels:
            raise ValueError(f'Nhãn không tồn tại: {label1 if label1 not in labels else label2}')
        start = labels[label1] + offset1
        end = labels[label2] + offset2
        diff = (end - start) & 0xFFFF
        # (Có thể in ra stderr để kiểm tra)
        # print(f'bytes_between: {label1}+{offset1} -> {label2}+{offset2} = {diff} (0x{diff:04X})', file=sys.stderr)
        result[pos] = diff & 0xFF
        result[pos + 1] = (diff >> 8) & 0xFF
...
def process_program(args, program, overflow_initial_sp):
    ...
    to the console.
    '''
    global result, labels, adr_of_cmds, adr_arith_cmds, pr_length_cmds, bytes_between_cmds, home, addr

    # Reset các danh sách lệnh hoãn
    bytes_between_cmds = []
...
"""
    ...
    # Process deferred adr_arith, bytes_between and pr_length commands
    finish_processing()
