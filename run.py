...
def ai_chat():
    ...
    try:
        ...
        if not OPENAI_API_KEY or OPENAI_API_KEY == 'your-api-key-here':
            return jsonify({'error': 'Chưa cấu hình OpenAI API key. Vui lòng thêm vào file .env'}), 501
        ...
        # Thêm lịch sử chat (tối đa 10 tin nhắn)
        ...
        return jsonify({'error': 'Lỗi xác thực OpenAI API. Kiểm tra API key trong file .env'}), 401
    except openai.error.RateLimitError:
        return jsonify({'error': 'Đã vượt quá giới hạn API. Thử lại sau hoặc nâng cấp tài khoản.'}), 429
    except openai.error.InvalidRequestError as e:
        return jsonify({'error': f'Lỗi request: {str(e)}'}), 400
...
<body>
    <div class="chat-container">
        <div class="chat-header">
            <div class="header-title">
                <span>🤖</span>
                AI Assistant
                <span class="ai-badge">Powered by ChatGPT</span>
            </div>
    ...
    <script>
        ...
        window.onload = function() {
            addBotMessage('Xin chào! Tôi là trợ lý AI thông minh của Casio Tool Server. Tôi có thể giúp gì cho bạn? Hãy hỏi về các công cụ HEX, ASM, Pixel, Spell, ROP...');
            loadHistory();
        };
...
if __name__ == "__main__":
    ...
    print("="*50)
    print(f"📌 Phiên bản: 2.0 (with OpenAI)")
    print(f"🌐 Địa chỉ: http://localhost:{PORT}")
    print(f"🤖 AI Chatbot: http://localhost:{PORT}/ai-chatbot")
    print(f"📁 Thư mục upload: {UPLOAD_FOLDER}")
    print(f"🔑 OpenAI API: {'✅ Có' if OPENAI_API_KEY and OPENAI_API_KEY != 'your-api-key-here' else '❌ Chưa cấu hình'}")
    print("="*50)
    
    if not OPENAI_API_KEY or OPENAI_API_KEY == 'your-api-key-here':
        print("\n⚠️  CẢNH BÁO: Chưa có OpenAI API key!")
